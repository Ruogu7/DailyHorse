...
public class MonitorService extends Service implements LocationListener {
    ...
    @Override public void onLocationChanged(Location l){if(!prefs.getBoolean("enabled",false))return;try{JSONArray places=new JSONArray(prefs.getString("places","[]"));String time=new SimpleDateFormat("HH:mm",Locale.US).format(new Date());String day=new SimpleDateFormat("yyyyMMdd",Locale.US).format(new Date());String morning=prefs.getString("morningTime","07:00"),evening=prefs.getString("eveningTime","17:40");double radius=Double.parseDouble(prefs.getString("radius","100"));double nearest=Double.MAX_VALUE;String nearestName="打卡地点";boolean nearAny=false;java.util.ArrayList<String> exits=new java.util.ArrayList<>();SharedPreferences.Editor state=prefs.edit();for(int i=0;i<places.length();i++){JSONObject p=places.optJSONObject(i);if(p==null)continue;String id=p.optString("id",String.valueOf(i));float[] d=new float[1];Location.distanceBetween(l.getLatitude(),l.getLongitude(),p.getDouble("lat"),p.getDouble("lon"),d);String name=p.optString("name","打卡地点");if(d[0]<nearest){nearest=d[0];nearestName=name;}if(d[0]<=radius)nearAny=true;String insideKey="inside_"+id;boolean wasInside=prefs.getBoolean(insideKey,d[0]<=80);boolean isInside=d[0]<=80;if(wasInside&&!isInside)exits.add(name);state.putBoolean(insideKey,isInside);}state.putLong("lastLocationAt",System.currentTimeMillis()).putFloat("lastLatitude",(float)l.getLatitude()).putFloat("lastLongitude",(float)l.getLongitude()).putFloat("lastNearestMeters",(float)nearest).putString("lastNearestName",nearestName).putString("monitorState","定位正常，最近地点 "+Math.round(nearest)+" 米").apply();if(time.compareTo(morning)>=0&&time.compareTo(evening)<0&&nearAny&&!day.equals(prefs.getString("morningDay",""))){prefs.edit().putString("morningDay",day).apply();alert("已到达打卡地点附近","morning","all");}for(String name:exits)alert(name+"：已走出 80 米范围","exit",name+System.currentTimeMillis());}catch(Exception e){setState("处理定位时出错："+e.getClass().getSimpleName());android.util.Log.e("DailyRecord","Location processing failed",e);}}
