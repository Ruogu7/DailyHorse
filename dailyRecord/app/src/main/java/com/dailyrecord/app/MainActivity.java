...
public class MainActivity extends Activity {
    ...
    void startMonitoring(){if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED){requestPerms();Toast.makeText(this,"请先授予精确定位权限",Toast.LENGTH_LONG).show();return;}if(Build.VERSION.SDK_INT>=29&&checkSelfPermission(Manifest.permission.ACCESS_BACKGROUND_LOCATION)!=PackageManager.PERMISSION_GRANTED){Toast.makeText(this,"请在应用设置中将位置权限设为“始终允许”",Toast.LENGTH_LONG).show();startActivity(new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS,Uri.parse("package:"+getPackageName())));return;}if(Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED){requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},11);Toast.makeText(this,"请允许通知；授权后再次点击启动提醒",Toast.LENGTH_LONG).show();return;}if(prefs.getString("places","[]").equals("[]")){Toast.makeText(this,"请先在设置中添加至少一个地点",Toast.LENGTH_LONG).show();startActivity(new Intent(this,SettingsActivity.class));return;}boolean wasEnabled=prefs.getBoolean("enabled",false);SharedPreferences.Editor edit=prefs.edit().putBoolean("enabled",true);if(!wasEnabled)edit.putBoolean("eveningStartupCheckPending",true);edit.apply();Intent i=new Intent(this,MonitorService.class);if(Build.VERSION.SDK_INT>=26)startForegroundService(i);else startService(i);refreshStatus();Toast.makeText(this,"提醒已启动",Toast.LENGTH_SHORT).show();}
